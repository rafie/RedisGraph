...
static AST_Validation _Validate_MATCH_Clause_ReusedIdentifiers(const cypher_astnode_t *node,
                                                const cypher_astnode_t *current_pattern,
    ...
    if (cypher_astnode_type(node) == CYPHER_AST_IDENTIFIER) {
        ...
        if (val != TRIEMAP_NOTFOUND && val != current_pattern) {
            asprintf(reason, "Alias '%s' reused. Entities with the same alias may not be referenced in multiple patterns.", alias);
            return AST_INVALID;
        }
        // Use the pattern pointer as a value, as it is valid to reuse a node alias within a pattern.
        TrieMap_Add(identifiers, (char*)alias, strlen(alias), (void*)current_pattern, TrieMap_DONT_CARE_REPLACE);
    }

    unsigned int child_count = cypher_astnode_nchildren(node);
    for (unsigned int i = 0; i < child_count; i++) {
        const cypher_astnode_t *child = cypher_astnode_get_child(node, i);
        if (_Validate_MATCH_Clause_ReusedIdentifiers(child, current_pattern, identifiers, reason) != AST_VALID) {
            return AST_INVALID;
        }
    }
    return AST_VALID;
